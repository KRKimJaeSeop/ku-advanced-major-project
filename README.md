...
### 2. 2단계 어텐션 메커니즘
- **Hourly Attention**: 하루 중 이상 행동이 집중된 시간대 도출
- **Temporal Attention**: 30일 중 이상 징후가 발현된 날짜 가중치 역산
- **해석 가능성**: 현장 복지사가 위기 원인을 구체적으로 파악 가능
...
- **직관적 시각화**: 
  - 최근 7일 24시간 가중치 2D 히트맵
  - 일별 중요도 Bar Chart
  - 야간 시간대 욕실 체류 시간 등 구체적 위기 원인 표시
- **알림 시스템**: 이메일 및 대시보드 실시간 전송
...
### AI 모델 성능 (완전히 새로운 환자 테스트)

| 지표 | 값 | 설명 |
|------|-----|------|
| **Precision** | 96.67% | 이상 알림 중 96.67%가 실제 이상 |
| **Recall** | **100%**  | 응급/사망을 100% 감지 |
| **F1 Score** | 98.31% | 거의 완벽한 균형 |
| **오탐률** | 6.9% | 평소 450건 중 31건만 오탐 |
| **False Negative** | **0건**  | 생명 위협 상황 놓치지 않음 |
...
- **학습**: 2500명 × 60일 (150,000행)
- **테스트**: 500명 × 60일 (3,000행)
- **총 환자**: 3000명
- **파트너**: 리본케어 (센서 데이터 샘플 제공)
...
| 이름 | 역할 | 담당 업무 |
|------|------|-----------|
| **김재섭**<br>(202415181) | AI 개발 / 프론트엔드 / 팀장 | Attention RNN 모델 설계 및 구현(PyTorch), 리본케어 데이터 전처리 파이프라인 구축(hex 디코딩 → 16개 특징 추출), MAE + z-score 기반 이상 감지 알고리즘 개발, 모델 성능 평가 및 검증(F1: 98.31%), 복지사 대시보드 UI/UX 설계, LLM 활용 개발 가속화 및 프로젝트 총괄 |
...
| 기능 분류 | 세부 기능 | 상태 | 완료율 |
|----------|----------|------|--------|
| **데이터 파이프라인** | 1시간 단위 시계열 윈도우 생성 | ✅ 완료 | 100% |
|  | StandardScaler 정규화 | ✅ 완료 | 100% |
| **AI 모델링** | Track A (Global) 오토인코더 학습 | ✅ 완료 | 100% |
|  | Track B (Personal) 파인튜닝 | ✅ 완료 | 100% |
|  | 2단계 어텐션 가중치 추출 | ✅ 완료 | 100% |
